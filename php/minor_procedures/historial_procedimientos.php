        ...
        <div class="container-fluid">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 animate__animated animate__fadeInDown">
                <div class="mb-3 mb-md-0">
                    <h2 class="fw-bold text-dark mb-1">Historial de Procedimientos</h2>
                    <p class="text-muted mb-0">Consulte y genere reportes de actividades</p>
                </div>
                <div>
                                        ...
                                        <tr class="align-middle">
                                            <td class="ps-4">
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="mb-0 text-sm fw-bold text-dark"><?php echo htmlspecialchars($proc['nombre_paciente']); ?></h6>
                                                </div>
                                            </td>
                                            <td>
                                                <p class="text-xs font-weight-bold mb-0 text-primary"><?php echo htmlspecialchars($proc['procedimiento']); ?></p>
                                            </td>
                                            <td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generatePDF(res) {
        ...
        // Header
        // Add logo if available (needs base64 or URL, difficult directly here without CORS issues or converting first. 
        // For now, we will use text header heavily styled)
        ...
        doc.autoTable({
            startY: 75,
            head: [['Paciente', 'Procedimiento', 'Fecha y Hora', 'Cobro', 'Usuario']],
            body: tableBody,
            theme: 'grid',
            headStyles: {
        ...
        // Save
        const fileName = `Reporte_Jornada_${document.getElementById('reportDate').value}.pdf`;
        doc.save(fileName);
        
        // Check if user wants to print (Logic: User asked for option to print. 
        // Browser download usually offers print, but we can try opening autoPrint in new window if needed.
        // For now, save() triggers download. We can offer a "Print" preview button separately or simpler: 
        // Just let them print the downloaded PDF.)
        // To strictly follow "option to print if doctor requires", we can create a Blob and open it.
        const pdfBlob = doc.output('bloburl');
         ...
         Swal.fire({
            ...
            if (result.isConfirmed) {
                window.open(pdfBlob, '_blank'); // Opens PDF in new tab where they can print
            }
        });
    }
});
